...
pub trait Graph {
    ...
    fn add_edge(&mut self, edge: (&str, &str, i32)) {
        ...
        let from_node_neighbours = self.adjacency_table_mutable().get_mut(from_node).unwrap();
        from_node_neighbours.push((to_node.to_string(), weight));

        let to_node_neighbours = self.adjacency_table_mutable().get_mut(to_node).unwrap();
